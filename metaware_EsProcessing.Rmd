...
---
This code take processes the raw data contained in "metaware_ESData_raw.xlsx" and "metaware_SurvData_raw.csv"
...
# calculate mot, opp, bel, and pre scores
a <- a %>% 
  rowwise() %>% 
  mutate(mot = 
           if_else(condition = is.na(v2.mot),
                   true = v1.mot,
                   false = v1.mot + v2.mot),
         
         bel = 
           if_else(condition = is.na(v2.bel),
                   true = v1.bel,
                   false = v1.bel + v2.bel),
...
Probably better to do some sort of pivot here
```{r eval = F}
DF <- DF %>% 
  select(id, vig.1, vig.2, es, es.var) 

DF.vig <- read.csv("vig.ratings.csv") %>% 
  select(-c(sd.mot, sd.opp))

# vig.1 join
DF.vig <- rename(DF.vig, 
                 vig.1 = vig,
                 vig.1.m = m.mot,
                 vig.1.o = m.opp) 
  
tmp <- full_join(DF, DF.vig,
                 by = "vig.1")
