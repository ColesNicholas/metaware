...
tmp <- DF %>% 
  ...
  # compute OR
  rowwise() %>% 
  mutate(or = (count.1 * (n.2 - count.2)) / #  p. 36, Equation 5.8
              ((n.1 - count.1) * count.2),
         log.or = log(or), #  p. 36, Equation 5.9
         log.or.var = (1 / count.1) + # #  p. 36, Equation 5.10
                      (1 / (n.2 - count.2)) +
                      (1 / (n.1 - count.1)) +
                      (1 / count.2),
         es = log.or * (sqrt(3) / pi), #  p . 47 Equation 7.1
         es.var = log.or.var * (3 / pi^2) #  p . 47 Equation 7.2
         ) %>% 
  ungroup()
...
tmp2 <- tmp %>% 
  ...
  add_row(name = "test2",
          ...
          es.var = 0) %>% 
  rowwise() %>% 
  mutate(es = log.or * (sqrt(3) / pi), #  p . 47 Equation 7.1
         es.var = log.or.var * (3 / pi^2)) #  p . 47 Equation 7.2
