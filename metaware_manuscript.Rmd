...
For estimating overall effects and testing moderators, we used three-level meta-analysis. Three-level meta-analysis accommodates dependent effect sizes by modeling three sources of variability: the sampling error of individual studies (level 1), variability within studies (level 2), and variability between studies (level 3). To estimate the overall effect size, we fit an intercept-only three-level meta-regression model. For moderator analyses, continuous and dummy-coded categorical moderators were separately entered into the meta-regression model. For categorical moderators, we used the models to estimate overall effect sizes within each subgroup of the moderator.
...
We used several approaches to assess and correct for potential publication bias in our overall effect size estimate.

First, we visually examined *funnel plots* of observed effect sizes against their standard errors, the latter which is an index of effect size precision. Without publication bias, funnel plot distributions typically resemble a funnel: relatively large studies estimate the effect with high precision, and effect sizes fan out in *both* directions as the studies become less precise. If, however, relatively imprecise non-significant findings are omitted from the scientific record (i.e., there is publication bias), the resulting distribution is often asymmetric/sloped. For our purposes, we examined funnel plots both with (a) all effect sizes, and (b) effect sizes that were aggregated in cases of non-independence.

Second, we conducted precision-effect tests in (a) a two-level meta-regression with robust variance estimates, and (b) a three-level meta-regression (citation). In precision-effect tests, the relationship between effect size and variability---which would be absent when there is no publication bias---is estimated and controlled for in a meta-regression model. This subsequently produces estimates of publication bias and the bias-corrected overall effect. Precision-effect tests are usually conducted in a two-level meta-regression and have exaggerated error rates when there are non-independent effect sizes. Nonetheless, Rodgers and Pustejovsky (2021) demonstrated that the method retains fairly good statistical properties when expanded to either (a) include robust variance estimates , or (b) include a third level that accommodates nested effects. We used both approaches in the current work.

Third, we used weight-function modeling with aggregated dependent effect sizes [@vevea1995general]. In weight-function modeling, weighted distribution theory is used to model biased selection behavior, which is then compared to unadjusted model. If the adjusted model provides increased fit, publication bias is a concern and the adjusted model can be used to estimate the bias-corrected overall effect size. Once again, weight-function modeling was designed for two-level meta-analysis and has exaggerated error rates when there are non-independent effect sizes. Nonetheless, Rodgers and Pustejovsky (2021) demonstrated that the method retains fairly good statistical properties when non-independent effect sizes are aggregated.

Fourth, we examined whether effect sizes differ among published and non-published records. To do so, we fit a three-level meta-regression with publication status entered as a dummy-coded predictor. Last, we used an approach by @mathur2020sensitivity to estimate the maximal publication bias required to shift the observed overall effect size estimate to zero. To adjust for non-independence, we used robust variance estimates for this approach.

we examined funnel plots, performed precision-effect tests, fit a weight-function model, compared

implemented three approaches: (1) modeling non-independent effect sizes using three-level meta-analysis, (2) adjusting for non-independent effect sizes using robust variance estimation (e.g. Coles et al., 2019a), and (3) aggregated non-independent effect sizes (e.g., Coles et al., 2019b.
...
Results indicated that, overall, explicit manipulations of demand characteristics create a small acquiescence effect, $d$ = `r overall$b`, 95% CI [`r overall$ci.lb`, `r overall$ci.ub`], $t$ = `r overall$zval`, $p$ `r printp(overall$pval)`. In other words, when explicitly told a hypothesis, participants responses tend to shift in a manner consistent with that hypothesis. That being said, the observed effects of demand characteristics were highly heterogeneous. The between-study ($\tau$ = `r sqrt(overall$sigma2[1])`) and within-study ($\sigma$ = `r sqrt(overall$sigma2[2])`) standard deviation of effects was approximately the same size of the overall effect. This suggests the presence of moderators.
