...
In total, we extracted `r overall$k` effect sizes from `r overall$n` studies from between the years `r DF.es$year %>% min()` and `r DF.es$year %>% max()`. The average coder rating of reporting quality, internal validity, and external validity was `r DF.es$report %>% mean() %>% round(2)`, `r DF.es$internal %>%  mean() %>% round(2)`, and `r DF.es$external %>%  mean() %>% round(2)` out of 1, respectively. The low external validity scores were driven by the reliance on non-representative sampling methods, a common practice in experimental psychology that is nonetheless identified as a threat to external validity in the DOWNS AND BLACK checklist.
