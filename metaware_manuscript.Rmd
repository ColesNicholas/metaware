...
[^3]: After completing the meta-analysis, we realized we included one record with a clinical population: patients who suffered pulmonary or cerebrovascular accidents [@lana1971subject]. Excluding their results does not change the general pattern of results.
...
[^6]: For effect size aggregation, we assumed a default dependent effect size correlation of $r$ = .50 but performed sensitivity analysis with $r$ = .10, .30, .50, .70, and .90. These sensitivity analyses did not change our overall conclusion about publication bias, so we do not discuss them.
...
We currently lack a satisfying explanation for our results. One possibility is that demand characteristics are not driven by motivation, opportunity, and belief in the experimenter's hypothesis. However, we find this explanation unlikely given the face validity of those proposed mechanisms. A second possibility is that these mechanisms matter – but in a way that is more complex than we were able to capture in the present review. For example, @corneille2022sixty notes that there are outcomes that may seem outside of participants control but can actually be changed via imagination. Similarly, @coles2022fact notes that mechanisms discussed in research on placebo effects (e.g., conditioned responses) may create changes in participants' responses – even with they apparently have neither the motivation nor opportunity to adjust responses. The existence of alternative mechanisms may mean that motivation and opportunity have a relatively weak relationship with demand effects that we were not able to reliably detect in our meta-analysis. A third possibility, of course, is that our own methodological limitations inhibited our ability to detect the effects of these moderators. We discuss these limitations next.
