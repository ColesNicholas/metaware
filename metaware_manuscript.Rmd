...
```{r modforest, fig.cap = "Selected moderator subgroup mean effect sizes (dots) and their 95% confidence intervals (error bars).", fig.height = 5.6, fig.width = 5, warning = F}
## list of moderators
...
```{r predplot, fig.cap = "Estimated overall effect of positive demand characteristics in a  classic experimental setting (in-person studies testing positive effects with volunteer student samples) and an online worker setting (online studies testing positive effects with paid non-student samples).", fig.height = 5, fig.width = 5, warning = F}
...
Using a web-based Qualtrics survey, a convenience sample of `r survey.dem$n` undergraduates from Stanford University reviewed 10 randomly-selected vignettes in exchange for course credit (`r survey.dem$gend[1]`% female; `r survey.dem$gend[3]`% male, `r survey.dem$gend[6]`% did not report gender; `r survey.dem$gend[2] + survey.dem$gend[4] + survey.dem$gend[5]`% transgender or gender non-conforming). `r survey.dem$ethnicity[7]`% of participants reported they were White/Caucasian, `r survey.dem$ethnicity[2]`% Asian, `r survey.dem$ethnicity[3]`% Black/African American, and `r survey.dem$ethnicity[4]`% Native Hawaiian or Other Pacific Islander. `r survey.dem$ethnicity[5]`% of participants indicated that there ethnicity could not be described by any single provided category, and `r survey.dem$ethnicity[6]`% did not report ethnicity. The average participant age was `r survey.dem$age.m` ($SD$ = `r survey.dem$age.sd`). Sample size was based on availability of resources (i.e., we collected as much data as possible in a single semester).
...
m <- ggplot(data = mot.df,
  ...
  # geom_label(aes(x = max(X.mot) - 1, 
  #                y = 1.5),
  #            label = substitute(paste0(beta, " = ",
  #                                      round(as.numeric(mod.r$mo$mod$beta['mot', ]), 2)))
  #            ) + 
