...
DF.s2.tmp <- DF.s2 %>% 
  ...
  # refactor to make it easier to interpret
  mutate(demand = factor(demand,
                         levels = c("pos", "nil")))

# re-run mod analyses
## mot now sig. Effect bigger among motivation (b = .10; was .03)
lmer(happy ~ trial + block.num + mot.c +
       (trial:mot.c) +
       (1 | sub),
     data = DF.s2.tmp) %>%
  summary()

## still not finding evidence of opp (b = .03)
lmer(happy ~ trial + block.num + opp.c +
       (trial:opp.c) +
       (1 | sub),
     data = DF.s2.tmp) %>%
  summary()

## still finding evidence of bel (b = .05)
lmer(happy ~ trial + block.num + bel.c +
       (trial:bel.c) +
       (1 | sub),
     data = DF.s2.tmp) %>%
  summary()
...
## trial : demand is pos, indicating that the trial effect gets bigger in pos demand
## trial : demand : mot is sig, indicating that demand becomes more effective as motivation grows.
## not robust to decisions about att check
tmp <- lmer(happy ~ trial * demand * mot.c +
       (1 | sub),
     data = DF.s2.tmp)
...
## trial : demand is pos, indicating that the trial effect gets bigger in pos demand
## trial : demand : bel is sig, indicating that demand becomes more effective as belief grows.
## robust to decisions about att check
tmp <- lmer(happy ~ trial * demand * bel.c +
       (1 | sub),
     data = DF.s2.tmp)

summary(tmp)

## opp still doesn't matter
tmp <- lmer(happy ~ trial * demand * opp.c +
       (1 | sub),
     data = DF.s2.tmp)
