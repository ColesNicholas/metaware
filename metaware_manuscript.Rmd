...
# estimate proportion of counter-acquiesence effects
c.acq.per <- pnorm(q = (-.10),
                   mean = overall$b,
...
Although demand characteristics produce small acquiescence effects *on average*, the magnitude and direction of these effects is not consistent (Forest plot figure). Indeed, the between-study ($\tau$ = `r sqrt(overall$sigma2[1])`) and within-study ($\sigma$ = `r sqrt(overall$sigma2[2])`) effect size standard deviation was approximately the same size of the overall effect. This suggests that demand characteristic sometimes produce negligible shifts in participants responses. This also suggests that demand characteristics sometimes produce *counteracquiescence*: participants shifting their response in the *opposite* direction of the hypothesis. This is shown in Figure X, which visualizes the distribution of demand characteristics effects based on the meta-analytic mean and standard deviation. For the sake of example, we characterized any shift less than 0.10 standard deviation as "negligible". Based on this characterization, `r acq.per` % of demand characteristics manipulations produce non-negligible acquiesence and `r c.acq.per` % produce non-negligible counteracquiesence.
...
Study 1 provides the first quantitative synthesis of strict experimental tests of demand effects. Overall, explicit manipulations of demand characteristics lead to a small acquiescence effect. However, significant heterogeneity was observed. Using arbitrary thresholds, we estimated that X% of demand characteristics manipulations produce significant acquiesence, X% produce significant counteracquiesence, and X% produce negligible shifts in participant responses.

Moderator analyses revealed two study-level features that are associated with stronger acquiesence effects: (1) working with student participant populations, and (2) researchers communicating that they expect *no* shift in participant responses (i.e., "nil demand" manipulations). Demand characteristics also tended to produce larger acquiesence effects when participants were paid---although this difference was not statistically significant.

Study 1 gives us some information about the consistency and potential contextual moderators of demand effects. However, they do not address outstanding questions regarding the extent to which these effects are driven by response bias vs. placebo effect. For example, consider our finding that students tended to acquiesence more than non-students. This may happen because students are more motivated to help the experimenter (response bias), but it may also happen because students are more likely to believe the communicated hypothesis (placebo effect). We turn our attention to this issue in Studies 1b and 2.
