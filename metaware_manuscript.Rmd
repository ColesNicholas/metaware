...
# for each moderator, conduct moderator and (if categorical) subgroup analyses
mod.r <- 
  sapply(X = mod.l,
         simplify = F,
         FUN = function(m) {
...
#### Other moderators
We did not find that the effects of demand characteristics were moderated by the following moderators: whether the researchers manipulated demand characteristics within- ($d$ = `r mod.r[mod.r$mod == "design" & !is.na(mod.r$level) & mod.r$level == "within", ]$d`) or between-subjects, 

used a within- (d = XX, 95% CI, p) or between-subjects design, F(df) = XX, p = XX); etc.
