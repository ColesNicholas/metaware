...
For each vignette, determine the proportion of participants who passed the attentino check. Then, remove participants who failed attention checks.
```{r}
surv.att <- DF.surv %>% 
  group_by(vig) %>% 
...
DF <- DF %>% 
  ...
  rowwise() %>% 
  mutate(att = sum(c(v1.att, v2.att),
                   na.rm = T),
         mot = sum(c(v1.mot, v2.mot), 
                   na.rm = T),
