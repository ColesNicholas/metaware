...
```{r}
# import data
DF.surv <- read_csv(file = "data/metaware_SurvData_test.csv") %>%
  
  # remove unnecessary variables
  select(-c(StartDate : UserLanguage),
         -c(hap1_bl1_hap : survey_order), # keep this in
         #-c(vig_assign, mot_gen : id), # drop this
         -contains("Click"),
         -contains("Submit"),
         -contains("Count")) %>% 
...
## [optional] look at reliability of ratings
```{r}
even <- DF.surv |>
...
### Alternative way of looking at icc
```{r}
...
# vignette ratings (passed attention checks)
DF.surv %>% 
  filter(!is.na(awr),
         att.chk == 1) %>% 
  group_by(vig) %>% 
...
# precision sample size estimate
prec <- sapply(
  X = c("mot", "opp", "bel", "pre"),
  USE.NAMES = T,
  FUN = function(x){
